...
export class DataApiService {
  ...
  constructor(
	...
	getAllTixs(){
		const url_api = 'https://db.techosytabiquesmerida.com:3028/api/tixes?filter[where][status]=activated';
		...
		getAllCategories(){
		const url_api = 'https://db.techosytabiquesmerida.com:3028/api/categories?filter[where][status]=activated';
		return this.http.get(url_api);
	}
 		getTamano(){
		const url_api = 'https://db.techosytabiquesmerida.com:3028/api/tixes?filter[where][status]=activated';
	...
	getTamanoIni(){
		const url_api = 'https://db.techosytabiquesmerida.com:3028/api/tixes?filter[where][initload]=activated';
		return (this.tixs = this.http.get(url_api));
	}
	...
	getInfo(){
		const url_api=`https://db.techosytabiquesmerida.com:3028/api/infos/`;
		this.info = this.http.get(url_api);
		return (this.info);
	}
	saveSale(sale :SaleInterface){
		const url_api='https://db.techosytabiquesmerida.com:3028/api/sale';
		return this.http
		.post<SaleInterface>(url_api, sale)
		.pipe(map(data => data));
	}
	saveOrder(order :OrderInterface){
		const url_api='https://db.techosytabiquesmerida.com:3028/api/order';
		return this.http
...
sendMailNewBookAppToAdmin(book){
	...
	updateOrder(order :OrderInterface, id: string){
		// let token = this.authService.getToken();
		const url_api=`https://db.techosytabiquesmerida.com:3028/api/order/${id}`;
		return this.http
		.put<OrderInterface>(url_api, order)
		.pipe(map(data => data));
	}
	getOrderByNpedido(npedido: string){
		const url_api = `https://db.techosytabiquesmerida.com:3028/api/order?filter[where][npedido]=${npedido}`;
